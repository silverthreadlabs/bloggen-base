...
export function ChatSidebar({ currentChatId: initialChatId }: Props) {
  ...
  const { pinnedChats, groupedChats, hasResults, totalChats } = useMemo(() => {
    if (!chats) return { pinnedChats: [], groupedChats: {}, hasResults: false, totalChats: 0 };
  ...
  return (
    <Sidebar collapsible="icon" className="border-r max-w-72 h-screen flex flex-col">
      <SidebarHeader className="p-2 shrink-0">
        ...
        <div className="flex items-center justify-center mb-2">
          ...
          >
            ...
            >
              {isCollapsed ? <Image src="/favicon/icon-512.png" alt="Bloggen Logo" width={32} height={32} /> : <LogoDark className="" />}
            </span>
          </div>
        </div>
      ...
      <SidebarContent className="flex-1 min-h-0">
        ...
        <div className={cn('group-data-[collapsible=icon]:hidden h-full flex flex-col')}>
          {/* Pinned Section */}
                              ...
                              <div className="px-2 py-1 text-xs font-medium text-muted-foreground flex items-center justify-between">
                                ...
                                {searchQuery && (
                                  <span className="text-xs">
                                    {monthChats.length} {monthChats.length === 1 ? 'chat' : 'chats'}
                                  </span>
                                )}
                              </div>
                              <SidebarMenu>
