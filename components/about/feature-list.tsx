...
function FeatureCard({
  ...
  color,
  gradient,
}: FeatureData & { index: number }) {
  return (
    <div className="group relative transform transition-all duration-500 ease-out hover:-translate-y-2">
      {/* Glow effect */}
      <div
        className={`absolute -inset-0.5 rounded-sm blur opacity-0 group-hover:opacity-10 transition-opacity duration-500`}
      ></div>

      {/* Main card */}
      <div
        className={`relative rounded-sm border border-canvas-line p-8 shadow-lg transition-all duration-500 group-hover:shadow-2xl overflow-hidden`}
      >
        ...
        <div className="flex flex-col z-10">
          ...
          <div className="flex items-center justify-between mb-6">
            <div
              className={`relative p-4 rounded-sm bg-gradient-to-br ${color} shadow-lg transform transition-all duration-500 group-hover:scale-110 group-hover:rotate-3`}
            >
              <div className="text-white">{icon}</div>
              {/* Icon glow */}
              <div
                className={`absolute inset-0 bg-gradient-to-br ${color} rounded-sm blur-md opacity-0 group-hover:opacity-40 transition-opacity duration-500 -z-10`}
              ></div>
            </div>
          ...
          <div className="text-canvas-text leading-relaxed space-y-2">
            <p className="transition-colors duration-300 group-hover:text-canvas-text-contrast">
              ...
              {code && (
                <span
                  className={`inline-flex items-center px-3 py-1 rounded-sm bg-canvas-bg border text-sm font-mono font-medium text-canvas-text-contrast mx-1 transition-all duration-300`}
                >
                  {code}
                </span>
              )}
