...
import { useChat } from '@/chat/lib/hooks';
...
export function ChatContainer({ chatId, isNewChat = false }: Props) {
    ...
    const { data: existingChat, isLoading, isError } = useChat(isNewChat ? undefined : chatId);

    // For new chats, render immediately without loading state
    if (isNewChat) {
        return (
            <ChatComponents.ChatInterface key={chatId} chatId={chatId} initialChat={undefined} isLoadingChat={false} />
        );
    }
    ...
    return (
        <ChatComponents.ChatInterface key={chatId} chatId={chatId} initialChat={existingChat} isLoadingChat={false} />
    );
}
