...
import { useFileUploads } from '@/chat/hooks/use-file-uploads';
import { deleteTrailingMessages } from '@/chat/lib/actions/chat-actions';
import { type ChatWithMessages, useChat as useChatQuery, useChats, useUpdateChatTitleInCache } from '@/chat/lib/hooks';
...
import { useMessageModifiers } from '../lib/hooks/use-message-modifiers';
import {
    LENGTH_MARKER_END,
    LENGTH_MARKER_START,
    TONE_MARKER_END,
...
export function ChatInterface({ chatId, initialChat, isLoadingChat = false }: Props) {
    ...
    const extractUserContext = useCallback((fullContext: string): string => {
        ...
        while (toneStartIdx !== -1) {
            ...
            if (toneEndIdx !== -1) {
                ...
                extracted =
                    extracted.substring(0, toneStartIdx) + extracted.substring(toneEndIdx + TONE_MARKER_END.length);
        ...
        async (messageId: string, newContent: string) => {
            ...
            if (nextAssistantIndex !== -1) {
                ...
                try {
                    ...
                    const messageContext = (messages[messageIndex].metadata as { context?: string } | undefined)
                        ?.context;
